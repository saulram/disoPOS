...
class ControladorIngresos{
	...
	static public function ctrCrearIngreso(){
		...
		if(isset($_POST["nuevaVenta"])){
			...
			$respuesta = ModeloIngresos::mdlIngresoNuevo($tabla, $datos);
			...
			if($respuesta == "ok"){
				...
				swal({
					  type: "success",
					  title: "Agregado a inventarion correctamente",
					  showConfirmButton: true,
					  confirmButtonText: "Imprimir"
					  }).then(function(result){
								if (result.value) {

									window.open("http://"+window.location.host+"/extensiones/tcpdf/pdf/ingreso-carta.php?codigo='.$datos["codigo"].'","_blank");
	...
	static public function ctrEditarIngreso(){
		...
		if(isset($_POST["editarVenta"])){
			...
			$traerVenta = ModeloIngresos::mdlMostrarIngresos($tabla, $item, $valor);
	...
	static public function ctrEliminarIngreso(){
		...
		if(isset($_GET["idVenta"])){
			...
			$traerVenta = ModeloIngresos::mdlEliminarIngreso($tabla, $item, $valor);
			...
			$traerVentas = ModeloIngresos::mdlMostrarIngresos($tabla, $itemVentas, $valorVentas);
			...
			$respuesta = ModeloIngresos::mdlEliminarIngreso($tabla, $_GET["idVenta"]);
	...
	public function ctrDescargarReporteIngresos(){

		if(isset($_GET["reporteingresos"])){

			$tabla = "ingresos";

			if(isset($_GET["fechaInicial"]) && isset($_GET["fechaFinal"])){

				$reporteingresos = ModeloIngresos::mdlRangoFechasIngresos($tabla, $_GET["fechaInicial"], $_GET["fechaFinal"]);
				...
				$reporteingresos = ModeloIngresos::mdlMostrarIngresos($tabla, $item, $valor);
			...
			foreach ($reporteingresos as $row => $item){ 
				...
				$descuento = $item["impuesto"] * 100 / $item["neto"];
				...
				foreach ($productos as $key => $valueProductos) {
				 ...
				 <td style='border:1px solid #eee;'>".$valueProductos["descripcion"]."</td>
				 <td style='border:1px solid #eee;'>".$valueProductos["cantidad"]."</td> 
				 <td style='border:1px solid #eee;'>% ".$descuento."</td>
			     <td style='border:1px solid #eee;'>$ ".$valueProductos["precio"]."</td>	
				 <td style='border:1px solid #eee;'>$ ".$valueProductos["precio"]*$valueProductos["cantidad"]."</td>
	...
	static public function ctrDescargarXML(){
		...
		if(isset($_GET["xml"])){
			...
			$ventas = ModeloIngresos::mdlMostrarIngresos($tabla, $item, $valor);
